...
class GaussianMF(nn.Module):
    ...
    def forward(self, x):
        ...
        sigma = nn.functional.relu(self.sigma)
...
class TriangularMF(nn.Module):
    ...
    def forward(self, x, delta=10e-8):
        ...
        deltaL = nn.functional.relu(self.deltaL)
        deltaR = nn.functional.relu(self.deltaR)
...
class ANFIS(nn.Module):
    def __init__(self, variables, mf_shape, and_operator=torch.prod, output_activation=nn.ReLU(), mean_rule_activation=False):
        ...
        Args:
            ...
            mf_shape:             str containing the shape of the fuzzy sets of the system. Supports 'triangular' and
                                  'gaussian'.
